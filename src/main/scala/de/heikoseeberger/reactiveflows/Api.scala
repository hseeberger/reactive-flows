...
import akka.http.scaladsl.model.StatusCodes.{ BadRequest, Conflict, Created, NoContent, NotFound, PermanentRedirect }
...
object Api {
  ...
  def route(flowFacade: ActorRef,
            flowFacadeTimeout: Timeout)(implicit ec: ExecutionContext) = {
            ...
            entity(as[AddMessageRequest]) {
              case AddMessageRequest(text) =>
                onSuccess(flowFacade ? AddMessage(flowName, text)) {
                  case bc: BadCommand   => complete(BadRequest -> bc)
                  case fu: FlowUnknown  => complete(NotFound -> fu)
                  case ma: MessageAdded => completeCreated(ma.message.id, ma)
                }
            }
          }
        }
