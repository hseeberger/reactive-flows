  ...
  "A HttpService" should {
    "successfully bind to a socket and terminate on receiving a Stop" in {
      ...
      val httpService = EventFilter.info(occurrences = 1, pattern = s"Listening on.*$address:$port").intercept {
        system.actorOf(HttpService.props(address, port, system.deadLetters, flowFacadeTimeout))
      }

      val otherHttpService = probe.watch(system.actorOf(HttpService.props(address, port, system.deadLetters, flowFacadeTimeout)))
