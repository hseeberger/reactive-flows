...
class HttpServiceSpec extends WordSpec with Matchers
    with RouteTest with TestFrameworkInterface.Scalatest with RequestBuilding with SprayJsonSupport with JsonProtocol {
  ...
  "A HttpService" should {
      ...
      flowFacade.setAutoPilot(new TestActor.AutoPilot {
        def run(sender: ActorRef, msg: Any) = msg match {
          case FlowFacade.GetFlows =>
            sender ! List(
              FlowFacade.FlowDescriptor("akka", "Akka"),
              FlowFacade.FlowDescriptor("angularjs", "AngularJS")
            )
    ...
    "ask the FlowFacade AddFlow and respond with Created with the proper payload upon a 'POST /flows' for an unknown Flow" in {
      ...
      flowFacade.setAutoPilot(new TestActor.AutoPilot {
        def run(sender: ActorRef, msg: Any) = msg match {
          case FlowFacade.AddFlow("Akka") =>
            sender ! FlowFacade.FlowAdded(FlowFacade.FlowDescriptor("akka", "Akka"))
            TestActor.NoAutoPilot
        }
      })
      val mediator = TestProbe()
      ...
      request ~> route(httpService.ref, flowFacade.ref, flowFacadeTimeout, mediator.ref, 99) ~> check {
        ...
        responseAs[String].parseJson shouldBe
          """|{
             |  "flowDescriptor": { "name": "akka", "label": "Akka" }
             |}""".stripMargin.parseJson
      }
    }
    ...
    "ask the FlowFacade AddFlow and respond with Conflict with the proper payload upon a 'POST /flows' for an existing Flow" in {
      ...
      flowFacade.setAutoPilot(new TestActor.AutoPilot {
        def run(sender: ActorRef, msg: Any) = msg match {
          case FlowFacade.AddFlow("Akka") =>
            sender ! FlowFacade.ExistingFlow("Akka")
            TestActor.NoAutoPilot
        }
      })
      val mediator = TestProbe()
    ...
    //            sender ! Source(List(
    //              FlowFacade.FlowAdded(FlowFacade.FlowDescriptor("akka", "Akka")),
    ...
    //        )
    //        def run(sender: ActorRef, msg: Any) = msg match {
    //          case PubSubMediator.Subscribe(Flow.MessageEventKey, subscriber) =>
    //            println(s"##### YES ##### $subscriber")
    //            events.foreach(subscriber ! _)
    //            TestActor.NoAutoPilot
    //        }
    //      })
