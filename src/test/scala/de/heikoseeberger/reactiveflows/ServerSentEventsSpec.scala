...
class ServerSentEventsSpec extends WordSpec with Matchers {
  ...
  "A FlowEvent" should {
    "be convertible to a ServerSentEvent, if it is a FlowAdded" in {
      ...
      val serverSentEvent = fromFlowEvent(FlowFacade.FlowAdded(flowDescriptor))
    ...
    "be convertible to a ServerSentEvent, if it is a FlowRemoved" in {
      val serverSentEvent = fromFlowEvent(FlowFacade.FlowRemoved("akka"))
  ...
  "A MessageEvent" should {
    "be convertible to a ServerSentEvent, if it is a MessageAdded" in {
      ...
      val serverSentEvent = fromMessageEvent(messageAdded)
