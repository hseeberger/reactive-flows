...
class FlowEventPublisherSpec extends BaseAkkaSpec {
  ...
  "A FlowEventPublisherSpec" should {
    ...
    "subscribe to flow events and publish those" in {
      ...
      val flowEvent = Source(ActorPublisher[FlowFacade.FlowEvent](flowEventPublisher)).runWith(Sink.head)
      flowEventPublisher ! FlowFacade.FlowAdded(FlowFacade.FlowInfo("akka", "Akka"))
      val FlowFacade.FlowAdded(FlowFacade.FlowInfo(name, label)) = Await.result(flowEvent, 1 second)
      name shouldBe "akka"
      label shouldBe "Akka"
    }
  }
