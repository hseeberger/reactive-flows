...
class ReactiveFlowsSpec extends BaseAkkaSpec {
  ...
  "ReactiveFlows" should {
    "terminate the system when its PubSubMediator child actor terminates" in {
      ...
      system.actorOf(ReactiveFlows(_.actorOf(terminatingActor), (context, _) =>
                context.actorOf(Props.empty), (context, _, _, _, _, _, _) =>
    ...
    "terminate the system when its Api child actor terminates" in {
      implicit val system = ActorSystem()
      system.actorOf(ReactiveFlows(_.actorOf(Props.empty), (context, _) =>
                context.actorOf(Props.empty), (context, _, _, _, _, _, _) =>
                context.actorOf(terminatingActor)))
      Await.ready(system.whenTerminated, 3.seconds)
    }

    "terminate the system when its PubSubMediator child actor fails" in {
      implicit val system = ActorSystem()
      system.actorOf(ReactiveFlows(_.actorOf(faultyActor), (context, _) =>
                context.actorOf(Props.empty), (context, _, _, _, _, _, _) =>
                context.actorOf(Props.empty)))
      Await.ready(system.whenTerminated, 3.seconds)
    }
    ...
    "terminate the system when its FlowFacade child actor fails" in {
      implicit val system = ActorSystem()
      system.actorOf(ReactiveFlows(_.actorOf(Props.empty), (context, _) =>
                context.actorOf(faultyActor), (context, _, _, _, _, _, _) =>
                context.actorOf(Props.empty)))
      Await.ready(system.whenTerminated, 3.seconds)
    }
